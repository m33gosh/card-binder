...
export function CardTile({ card, photoUrl }: { card: CardRow; photoUrl?: string }) {
  ...
  return (
    <Link to={`/cards/${card.id}`} className="card-tile">
      ...
      <div className="meta">{[card.quantity > 1 && `×${card.quantity}`, card.set_name, card.card_number && `#${card.card_number}`].filter(Boolean).join(' ')}</div>
    </Link>
  )
}
